...
SRC_URI="ftp://ftp.digium.com/pub/asterisk/old-releases/${MY_P}.tar.gz"
LICENSE="GPL-2"
...
IUSE="alsa curl debug freetds h323 imap jabber kde keepsrc misdn mmx mysql newt nosamples odbc oss pic postgres qt3 radius snmp speex ssl sqlite static vorbis zaptel zlib"
...
pkg_setup() {
	...
	# imap requires ssl
	if use imap && !use ssl ; then
		ewarn "imap useflag needs ssl enabled too, forcing ssl support"
	fi
...
src_unpack() {
	...
	#
	epatch "${FILESDIR}"/${PN}-1.4.0-configure-gentoo.diff
...
src_install() {
	...
	newinitd "${FILESDIR}"/1.4.0/asterisk.rc6 asterisk
